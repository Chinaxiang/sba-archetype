...
#==========================================================================================
# JVM Configuration
# -Xmx512m:设置JVM最大可用内存为512m,根据项目实际情况而定，建议最小和最大设置成一样。
...
#==========================================================================================
JAVA_OPT="-server -Xms512m -Xmx512m -Xmn256m -XX:MetaspaceSize=64m -XX:MaxMetaspaceSize=320m"
JAVA_OPT="${JAVA_OPT} -XX:-OmitStackTraceInFastThrow -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=${LOG_DIR}/heap_dump.hprof"
...
# 输出项目PROFILE
STARTUP_LOG="${STARTUP_LOG}application profile: ${PROFILE}\n"
# 输出项目名称
...
# 打印启动命令
STARTUP_LOG="${STARTUP_LOG}application startup command: nohup java ${JAVA_OPT} -jar ${BASE_PATH}/boot/${APPLICATION_JAR} --logging.config=${CONFIG_DIR}logback-spring.xml --spring.config.location=${CONFIG_DIR} --spring.profiles.active=${PROFILE} > ${LOG_PATH} 2>&1 &\n"


#======================================================================
# 执行启动命令：后台启动项目,并将日志输出到项目根目录下的logs文件夹下
#======================================================================
nohup java ${JAVA_OPT} -jar ${BASE_PATH}/boot/${APPLICATION_JAR} --logging.config=${CONFIG_DIR}logback-spring.xml --spring.config.location=${CONFIG_DIR} --spring.profiles.active=${PROFILE} > ${LOG_PATH} 2>&1 &
